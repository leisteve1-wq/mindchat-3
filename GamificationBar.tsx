...
export default function GamificationBar({ onClose }: GamificationBarProps) {
  ...
  const xpProgress = Math.min((user.stats.xp / xpForNextLevel) * 100, 100);
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {recentAchievements.length > 0 && (
              <button onClick={() => setShowAchievements(!showAchievements)} className="flex items-center gap-1 px-2 py-1 rounded-lg bg-white/10 hover:bg-white/20 transition-colors">
      ...
      {showAchievements && (
        <div className="absolute top-14 right-4 z-50 w-72 bg-[var(--mc-bg-secondary)] border border-[var(--mc-border)] rounded-xl shadow-xl">
          <div className="p-4">
            <h3 className="font-semibold mb-3 flex items-center gap-2"><Trophy className="w-5 h-5 text-yellow-400" />Recent Achievements</h3>
            <div className="space-y-2">
              {recentAchievements.map((achievement) => achievement && (
                <div key={achievement.id} className="flex items-center gap-3 p-2 rounded-lg bg-[var(--mc-bg-tertiary)]">
            ...
            <button onClick={() => setShowAchievements(false)} className="w-full mt-3 py-2 text-sm text-[var(--mc-text-muted)] hover:text-white transition-colors">Close</button>
          </div>
        </div>
      )}
    </>
